    ...
    */
    ...
    private func signalEnumeratorAfterAccountSetup() {
        guard let fpManager = NSFileProviderManager(for: domain) else {
            logger.error(
                "Could not get file provider manager for domain \(self.domain.displayName), cannot notify after account setup"
            )
            return
        }
        ...
        logger.debug(
            "Signalling enumerators for user \(self.ncAccount!.username) at server \(self.ncAccount!.serverUrl)"
        )
    ...
    func notifyChange() {
        guard let fpManager = NSFileProviderManager(for: domain) else {
            logger.error(
                "Could not get file provider manager for domain \(self.domain.displayName), cannot notify changes"
            )
            return
        }
        ...
        fpManager.signalEnumerator(for: .workingSet) { error in
            if error != nil {
                logger.error(
                    "Error signalling enumerator for working set, received error: \(error!.localizedDescription)"
                )
            }
        }
    }
    ...
    @objc func setupDomainAccount(
        ...
        guard account != ncAccount, user != "", userId != "", serverUrl != "", password != "" else {
            logger.warning(
                ...
                Received repeated or invalid user account details.
                    ...
                    ncKitAccount: \(account.ncKitAccount)
                """
            )
            return
        }
        ...
        Task {
            ...
            for authTimeout in AuthenticationTimeouts {
                ...
                logger.info(
                    "\(user) authentication try timed out. Trying again soon."
                )
            ...
            case .authenticationError:
                logger.info(
                    "\(user) authentication failed due to bad creds, stopping"
                )
                return
            case .connectionError:
                ...
                logger.info(
                    "\(user) authentication try failed, no connection."
                )
                return
            case .success:
                logger.info(
            ...
            Task { @MainActor in
                ncAccount = account
                dbManager = FilesDatabaseManager(account: account, log: log)
